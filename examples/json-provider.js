...
async function runJsonProviderExample() {
  ...
  console.log(`✅ Database created with provider: ${db.provider}, backend: ${db.backend}\n`);

  // Store user documents
  const users = [
    { id: 'user1', name: 'Alice Johnson', age: 28, department: 'Engineering', salary: 75000 },
    { id: 'user2', name: 'Bob Smith', age: 34, department: 'Marketing', salary: 65000 },
    { id: 'user3', name: 'Carol Davis', age: 29, department: 'Engineering', salary: 80000 },
    { id: 'user4', name: 'David Wilson', age: 31, department: 'Sales', salary: 60000 },
    { id: 'user5', name: 'Eve Brown', age: 26, department: 'Engineering', salary: 72000 }
  ];
  ...
  console.log('Engineers:', engineers.map(r => ({ key: r.key, name: r.value.name })));

  // Find users older than 30
  const seniorUsers = await db.find('users', { age: { $gt: 30 } });
  console.log('Senior users (>30):', seniorUsers.map(r => ({ key: r.key, name: r.value.name, age: r.value.age })));

  // Find users with salary between 70k and 80k
  const midSalaryUsers = await db.find('users', { 
    salary: { $gte: 70000, $lt: 80000 } 
  });
  console.log('Mid-salary users (70k-80k):', midSalaryUsers.map(r => ({ 
    key: r.key, 
    name: r.value.name, 
  ...
  console.log('Tech & Sales users:', techAndSales.map(r => ({ 
    key: r.key, 
    name: r.value.name, 
  ...
  console.log('Top 3 earners:', topEarners.map(r => ({ 
    name: r.value.name, 
    salary: r.value.salary 
  })));
  ...
  await db.batch([
    { type: 'set', table: 'users', key: 'user6', value: { id: 'user6', name: 'Frank Miller', age: 35, department: 'HR', salary: 55000 } },
    { type: 'set', table: 'settings', key: 'notifications', value: { email: true, push: false } },
  ...
  console.log('Promoted Alice:', { name: promotedAlice.name, salary: promotedAlice.salary, title: promotedAlice.title });
