  ...
  async initializeDatabases(): Promise<void> {
    try {
      ...
      try {
        ...
        console.warn(
          'SQL Database not available in Chrome extension service worker context, using JSON provider as fallback:',
          error,
        )
      ...
      switch (action) {
        ...
        case 'transaction':
          const result = await db.transaction('readwrite', async (tx: any) => {
            ...
            await tx.set(message.table || 'default', 'tx-key-1', {
              data: 'Transaction data 1',
              timestamp: Date.now(),
            })
            await tx.set(message.table || 'default', 'tx-key-2', {
        ...
        case 'bulkInsert':
          if (message.data && Array.isArray(message.data)) {
            ...
            return {
              success: true,
              data: `Inserted ${message.data.length} items`,
            }
          }
