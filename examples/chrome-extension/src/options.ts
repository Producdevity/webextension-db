...
function sendMessage(
  ...
  return new Promise((resolve) => {
    chrome.runtime.sendMessage(
      { action, provider: currentProvider, ...data },
      resolve,
    )
  })
}
...
const jsonOperations = {
  async set(): Promise<void> {
    const table = (document.getElementById('json-table') as HTMLInputElement)
      .value
    const key = (document.getElementById('json-key') as HTMLInputElement).value
    const valueText = (
      document.getElementById('json-value') as HTMLTextAreaElement
    ).value
  ...
  async generateSampleData(): Promise<void> {
    ...
    const sampleData = [
      ...
      {
        key: 'user3',
        value: { name: 'Carol Davis', age: 22, city: 'Chicago', active: true },
      },
    ]
...
const sqliteOperations = {
  async set(): Promise<void> {
    const table = (document.getElementById('sqlite-table') as HTMLInputElement)
      .value
    const key = (document.getElementById('sqlite-key') as HTMLInputElement)
      .value
    const valueText = (
      document.getElementById('sqlite-value') as HTMLTextAreaElement
    ).value
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.querySelectorAll('.provider-tab').forEach((tab) => {
    tab.addEventListener('click', (e) => {
      const provider = (e.target as HTMLElement).getAttribute(
        'data-provider',
      ) as 'json' | 'sqlite'
      if (provider) {
